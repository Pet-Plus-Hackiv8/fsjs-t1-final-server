...
export const petResolvers = {
  Query: {
    ...
    async fetchPet(parent, { UserId, id }) {
        try {
          // let inMemory = await redis.get(`pets:${id}`);
          // if (inMemory) {
          //   // console.log(JSON.parse(inMemory))
          //   return JSON.parse(inMemory);
          // }
        //   console.log(UserId, id, "INI ID")
        ...
        //   console.log(pet, "+_+_+_+");
          ...
          pet.Owner= user
        //   console.log(pet, ">><<<");
               // await redis.set("`pets:${id}`", JSON.stringify(pet));
          return pet;
        } catch (error) {
            // console.log(error.response.data);
            throw new GraphQLError(error.response.data.message)
  ...
  Mutation: {
    async addPet(parent,{name, imgUrl, gender,  species, breed, description, weight, UserId}) {
        try {
    ...
    async editPet(parent, {name, imgUrl, gender,  species, breed, description, weight, UserId, editId}) {
        try {
